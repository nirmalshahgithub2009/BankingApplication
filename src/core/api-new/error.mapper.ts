...
export const mapApiError = (error: unknown): ApiError => {
  ...
  if (!error) {
    return {
      code: 'NETWORK_ERROR',
      message: ERROR_MESSAGE_MAP['NETWORK_ERROR'] ?? 'Network connection failed.',
      statusCode: 0,
      timestamp,
    };
  }
  ...
  const errorCode: string = (responseData?.code as string | undefined) || `HTTP_${status}`;
  const errorMessage: string =
    ((ERROR_MESSAGE_MAP[errorCode] as string | undefined) ||
      (responseData?.message as string | undefined) ||
      (STATUS_CODE_MESSAGE_MAP[status] as string | undefined) ||
